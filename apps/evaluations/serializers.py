...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Question
        fields = ['id', 'form_id', 'text', 'input_type', 'order',
                  'weight', 'is_required', 'min_value', 'max_value']
        read_only_fields = ['id']
...
class QuestionCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Question
        fields = ['id', 'text', 'input_type', 'order',
                  'weight', 'is_required', 'min_value', 'max_value']
        read_only_fields = ['id']

    def validate_input_type(self, value):
        """Validate input type is allowed"""
        allowed_types = ['rating', 'text', 'dropdown',
...
class AssignmentRuleSerializer(serializers.ModelSerializer):
    ...
    evaluator_unit_name = serializers.CharField(
        source='evaluator_unit.name', read_only=True, default=None)
    evaluator_position_name = serializers.CharField(
        source='evaluator_position.name', read_only=True, default=None)
    evaluatee_unit_name = serializers.CharField(
        source='evaluatee_unit.name', read_only=True, default=None)
    evaluatee_position_name = serializers.CharField(
        source='evaluatee_position.name', read_only=True, default=None)
...
class EvaluationAssignmentSerializer(serializers.ModelSerializer):
    ...
    evaluator_email = serializers.CharField(
        source='evaluator_id.email', read_only=True)
    evaluatee_email = serializers.CharField(
        source='evaluatee_id.email', read_only=True)
...
class ResponseSerializer(serializers.ModelSerializer):
    ...
    question_text = serializers.CharField(
        source='question_id.text', read_only=True)

    class Meta:
        model = Response
        fields = ['id', 'assignment_id', 'question_id',
                  'question_text', 'score_value', 'text']
        read_only_fields = ['id']
